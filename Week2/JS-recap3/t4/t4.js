...
const success = (v) => {
  const myLoc = [v.coords.latitude, v.coords.longitude];

  restaurants.sort(
    (a, b) =>
      distanceToPoint(a.location.coordinates, myLoc) -
      distanceToPoint(b.location.coordinates, myLoc),
  );
  ...
  restaurants.forEach((i) => {
    ...
    restaurantName.textContent = i.name;
    const restaurantAddress = document.createElement("td");
    restaurantAddress.textContent = i.address;
    const restaurantCity = document.createElement("td");
    restaurantCity.textContent = i.city;

    tableRow.append(restaurantName, restaurantAddress, restaurantCity);
...
const coordinates = window.navigator.geolocation.getCurrentPosition(success);
